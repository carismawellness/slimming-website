// Usage: node tools/tmp-qa-st-ypos2.mjs <url>
...
const out = await p.evaluate(() => {
  ...
  const targets = [
    '4-in-1 skin tightening protocol',
    'tighten loose skin and smooth stubborn areas — without surgery',
  ...
  for (const t of targets) {
    ...
    document.querySelectorAll('h1,h2,h3,h4,h5,h6,p,span,div').forEach((el) => {
      const tt = norm(el.textContent);
      if (tt !== t && tt !== t + '.') return;
      const r = el.getBoundingClientRect();
      if (r.height === 0 || r.height > 300) return;
      const y = Math.round(r.top + window.scrollY);
      if (best === null || y < best) best = y;
    });
    found[t] = best;
  }
  // dual heading partial
  let dual = null;
  document.querySelectorAll('h1,h2,h3,h4,h5,h6,p,span,div').forEach((el) => {
    const t = norm(el.textContent);
    if (!t.startsWith('malta’s most effective') && !t.startsWith("malta's most effective")) return;
    const r = el.getBoundingClientRect();
    if (r.height === 0 || r.height > 200) return;
    const y = Math.round(r.top + window.scrollY);
