      ...
      <div className="cx-wrap cx-wrap-tight" style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'grid', gap: 'clamp(28px,5vw,56px)' }} className="cx-problem-grid">
          <Reveal>
            ...
            <h2 id="problem-heading" className="cx-h2" style={{ marginBottom: 20 }}>Why diets alone don&rsquo;t work — and what does</h2>
            <p className="cx-lead" style={{ lineHeight: 1.625 }}>{PROBLEM.body}</p>
          </Reveal>
          ...
          <Reveal stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
            {PROBLEM.pains.map((p) => (
              <div key={p.t} className="cx-card" style={{ padding: '20px 18px', borderRadius: 18 }}>
                <span aria-hidden style={{ width: 30, height: 30, borderRadius: 9, background: 'rgba(155,141,131,0.14)', marginBottom: 12, display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden><path d="M18 6L6 18M6 6l12 12" stroke="#9b8d83" strokeWidth="2" strokeLinecap="round" /></svg>
                </span>
                <h3 style={{ fontFamily: 'var(--wide)', fontSize: 12.5, letterSpacing: '0.04em', textTransform: 'uppercase', color: '#4f7256', marginBottom: 7 }}>{p.t}</h3>
