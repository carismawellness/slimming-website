  ...
  return (
    <nav className={`cx-nav ${scrolled ? 'scrolled' : ''}`}>
      ...
      <div className="cx-wrap" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: scrolled ? 64 : 78, transition: 'height .4s var(--ease)' }}>
        <a href="#top" aria-label="Carisma Slimming home" style={{ display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <a href={CONTACT.phoneHref} className="cx-navlink cx-phone" style={{ display: 'none' }}>{CONTACT.phone}</a>
          ...
          >
            <span style={{ width: 22, height: 2, background: 'var(--sage-ink)', borderRadius: 2, transition: 'transform .35s var(--ease)', transform: open ? 'translateY(7px) rotate(45deg)' : 'none' }} />
            <span style={{ width: 22, height: 2, background: 'var(--sage-ink)', borderRadius: 2, opacity: open ? 0 : 1, transition: 'opacity .25s' }} />
            <span style={{ width: 22, height: 2, background: 'var(--sage-ink)', borderRadius: 2, transition: 'transform .35s var(--ease)', transform: open ? 'translateY(-7px) rotate(-45deg)' : 'none' }} />
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ marginTop: 26, display: 'flex', flexDirection: 'column', gap: 14, alignItems: 'flex-start' }}>
          ...
          <a href={CONTACT.phoneHref} className="cx-link-underline" style={{ fontFamily: 'var(--body)', color: 'var(--taupe)', fontSize: 16 }}>
            Call {CONTACT.phone}
          </a>
        </div>
      </div>
